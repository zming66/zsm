#!/bin/sh
# Alpine Linux 适配版 sing-box 防火墙规则脚本
# 位置：/etc/sing-box/scripts/firewall.sh
...
if [ "$MODE" = "TProxy" ]; then
    ...
    nft add table inet sing-box 2>/dev/null
...
EOF
    ...
    echo "✅ TProxy 模式防火墙规则应用完成"

else
    echo "当前模式为 TUN 模式，不需要防火墙规则。"
fi
